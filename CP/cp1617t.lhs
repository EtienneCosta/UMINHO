...
\begin{code}
inv x = p2 . (for (split (split (p1 . p1) ((uncurry (*)) . p1)) ((uncurry (+)) . ((uncurry (*)) >< id))) ((1-x,1),1)) 

--quickCheck
--property
prop_inv :: Double -> Int -> Bool
prop_inv x n = abs (inv x n - 1/x) < 0.5
--test
test_inv = forAll (Test.QuickCheck.choose (1,2)) $ 
           \x -> forAll (Test.QuickCheck.choose (0,1000)) $ 
           \n -> prop_inv x n
\end{code}
...
wrapper = p1
worker = cataList (split (either (const 0) (cond (sep.p1) (p1.p2) (cond (p2.p2) (succ.p1.p2) (p1.p2)))) (either (const True) (sep.p1)))

sep c = (c ==' ' || c =='\n' || c =='\t')

prop_wc :: [Char] -> Bool
prop_wc s = (wc_w_final s) == (wc_w s)
...
\start
    |lcbr
        (lcbr (wc [] = 0) 
              (wc (c:l) = [wc l,[succ . wc l, wc l]. (lookahead l)?] . (sep c)?))
        (lcbr (lookahead [] = True)
              (lookahead (c:l) = sep c))|
\just{|<=>|}{Def-cond, Def-cons, Def-nil, Def-zero e Cancelamento-x}
    |lcbr
        (lcbr (wc . nil = zero)
              (wc . cons (c,l) = [wc l,[succ . wc, wc] . lookahead? l] . (sep c)?))
        (lcbr (lookahead . nil = const True)
              (lookahead . cons (c,l) = sep . p1 (c,l)))|
%
\just{|<=>|}{Def-cond, Igualdade Extensional e Cancelamento-x}
    |lcbr
        (lcbr (wc . nil = zero)
              (wc . cons (c,l) = [wc . p2,[succ . wc . p2, wc . p2] . (lookahead . p2)?] . (sep . p1)? (c,l)))
        (lcbr (lookahead . nil = const True)
              (lookahead . cons = sep . p1))|
%
\just{|<=>|}{Igualdade Extensional e Eq-+}
    |lcbr
        (wc . [nil,cons]= [zero, [wc . p2,[succ . wc . p2, wc . p2] . (lookahead . p2)?] . (sep . p1)?])
        (lookahead . [nil,cons] = [const True, sep . p1])|
%
\just{|<=>|}{Def-in}
    |lcbr 
        (wc . in= [zero, [wc . p2,[succ . wc . p2, wc . p2] . (lookahead . p2)?] . (sep . p1)?])
        (lookahead . in = [const True, sep . p1])|
%
\just{|<=>|}{Natural-p1, Natural-p2 e Cancelamento-x}
    |lcbr 
        (wc . in = [zero, [wc . p2,[succ . p1 . p2 . (id >< <wc,look>), p1 . p2 . (id >< <wc,look>)] . (p2 . p2 . (id >< <wc,look>))?] . (sep . p1)?])
        (lookahead . in = [const True, p1 . (sep >< <wc,look>)])|
%
\just{|<=>|}{Absorçao-+ e Natural-guarda; Natural-id e Functor-x}
    |lcbr 
        (wc . in= [zero, [wc . p2,[succ . p1 . p2, p1 . p2] . (p2 . p2)? . (id >< <wc,look>)] . (sep . p1)?])
        (lookahead . in = [const True, p1 . (sep >< id) . (id >< <wc,look>)])|
%
\just{|<=>|}{Natural-p2 e Cancelamento-x; Natural-const e Absorçao-+}
    |lcbr 
        (wc . in= [zero, [p1 . p2 . (id >< <wc,look>),[succ . p1 . p2, p1 . p2]. (p2 . p2)? . (id >< <wc,look>)] . (sep . p1)?])
        (lookahead . in = [const True, p1 . (sep >< id)] . (id + (id >< <wc,look>)))|
%
\just{|<=>|}{Natural-p1; Functor-Listas, Natural-p1}
    |lcbr 
        (wc . in= [zero, [p1 . p2 . (id >< <wc,look>),[succ . p1 . p2, p1 . p2]. (p2 . p2)? . (id >< <wc,look>)] . (p1 . (sep >< <wc,look>))?])
        (lookahead . in = [const True, sep . p1] . F<wc,look>)|
%
\just{|<=>|}{Functor-x, Natural-id, Absorçao-+ e Natural-guarda}
    |lcbr 
        (wc . in= [zero, [p1 . p2,[succ . p1 . p2, p1 . p2]. (p2 . p2)?] . (sep . p1)? . (id >< <wc,look>)])
        (lookahead . in = [const True, sep . p1] . F<wc,look>)|
%
\just{|<=>|}{Natural-const e Absorçao-+}
    |lcbr 
        (wc . in= [zero, [p1 . p2,[succ p1 . p2, p1 . p2] . (p2 . p2)?] . (sep . p1)?] . (id + (id >< <wc,look>)))
        (lookahead . in = [const True, sep . p1] . F<wc,look>)|
%
\just{|<=>|}{Functor-Listas}
    |lcbr 
        (wc . in = [zero, [p1 . p2,[succ . p1 . p2, p1 . p2]. (p2 . p2)?] . (sep . p1)?] . F<wc,look>)
        (lookahead . in = [const True, sep . p1] . F<wc,look>)|
\just{|<=>|}{Fookinga}
    |<wc, lookahead> = | \cata{|<[zero, [p1 . p2,[succ . p1 . p2, p1 . p2] . (p2 . p2)?] . (sep . p1)?],[const True, sep . p1]>|}
%
\just{<=}{Cancelamento-x e Leibniz}
    |wc = p1 . | \cata{|<[zero, [p1 . p2,[succ . p1 . p2, p1 . p2] . (p2 . p2)?] . (sep . p1)?],[const True, sep . p1]>|}
%
\end{eqnarray*}
\normalsize
...
outB_tree Nil = i1 ()
outB_tree (Block arv ((x,y):t))= i2 (arv,(x,y):t) 
...
inordB_tree = cataB_tree (either nil (conc.(id><(concat.(map(conc.(singl><id)))))))

largestBlock = cataB_tree (either (const 0) ((uncurry max).(id >< ((uncurry max).(length >< maximum).unzip))))

mirrorB_tree = anaB_tree ((const () -|- (f2l . (id >< reverse))) . outB_tree)
 
right2last :: [(a,b)] -> [(a,b)]
right2last [] = []
right2last [a] = [a]
right2last ((x1,x2):((y1,y2):t)) = (x1,y2): right2last ((y1,x2):t)
 
f2l = split (p2 . head . p2) (right2last . cons . (split (split (p1 . head . p2) p1) (tail . p2)))

lsplitB_tree [] = i1 ()
lsplitB_tree (h:t) = i2(partitionL h t)

partitionL h [] = ([],[(h,[])])
partitionL h (x:xs) = (lt, [(lP, ltg),(gP, gts)])
    where
        lP = min h x
        gP = max h x
        lt = filter (<=lP) xs
...
dotB_tree = dotpict . bmap nothing (Just . concat . (intersperse "|") . (fmap show)) . cB_tree2Exp

cB_tree2Exp = cataB_tree (either (const (Var "Nil")) ((uncurry Term) . (split ((fmap p1) . p2) (cons . (split p1 ((fmap p2) . p2))))))
...
\begin{code}
generateAlgae = anaA geraA geraB 

geraA = (const () -|- split id id) . outNat
geraB = (const () -|- id) . outNat

showAlgae = cataA (either (const "A") conc) (either (const "B") id)

--QuickCheck
--property
prop_Algae n = (length . showAlgae . generateAlgae) n == (fromInteger . fib . toInteger . succ) n
--test
test_Algae = forAll (Test.QuickCheck.choose (0,20)) $ \n -> prop_Algae n
\end{code}
...
\begin{code}
permuta [] = return []
permuta l = do{(r1,l1)<-getR l; l2 <- permuta l1; return (r1:l2)}

eliminatoria = p2 . (cataLTree (either (split id certainly) ((split (winner . p1) updateScores) . (split (jogo . (p1><p1)) (p2><p2)))))

updateScores r = D (r1++r2)
                where
                    r1 = map (id >< (*(p2 x))) l1
                    r2 = map (id >< (*(p2 y))) l2
                    (l1,l2) = ((split (unD . p1) (unD . p2)) . p2) r
                    (x:(y:t)) = unD (p1 r)

winner = (uncurry (!!)) . (id >< (aux . (uncurry elemIndex) . (split maximum id))) . unzip . unD
        where 
            aux Nothing = 0
            aux (Just x) = x
